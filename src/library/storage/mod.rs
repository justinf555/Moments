...
///
/// Backends receive an already-validated [`Bundle`], an [`EventSender`],
...
pub trait LibraryStorage: Send + Sync + 'static {
    ...
    ///
    /// Implementations should send [`AppEvent::Ready`](crate::app_event::AppEvent::Ready)
    /// once initialisation is complete.
    async fn open(bundle: Bundle, events: EventSender, tokio: Handle) -> Result<Self, LibraryError>
    where
        Self: Sized;
    ...
    /// finishing their current item. Sends [`AppEvent::ShutdownComplete`](crate::app_event::AppEvent::ShutdownComplete)
    /// before returning.
